...
| `ScrollFade` | builder | which edges fade, or `none` | Content fades towards the edges it is scrolled past without a backdrop-colored overlay. All primitives fade by default; `text_only` attenuates glyphs, emoji, underlines, and strikethroughs while surfaces, borders, paths, images, SVG icons, shadows, sprites, and glass remain unchanged. The edges are caller-owned: a region that hides nothing fades at neither edge and publishes `none` |
