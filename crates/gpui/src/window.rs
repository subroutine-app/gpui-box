...
pub struct Window {
    ...
    /// The stable identity behind `captured_hitbox`, when the capturing
...
impl Window {
    pub(crate) fn new(
        ...
        let mut web_a11y_action_receiver = None;
        ...
        if !accessibility_force_disabled {
            ...
            #[cfg(target_family = "wasm")]
            {
                web_a11y_action_receiver = Some(action_receiver);
            }
        }
        ...
        platform_window.on_request_frame(Box::new({
            ...
            move |request_frame_options| {
                ...
                #[cfg(target_family = "wasm")]
                if let Some(receiver) = &web_a11y_action_receiver {
                    let removed = match handle.update(&mut cx, |_, window, cx| {
                        if !window.removed {
                        ...
                        Ok(removed) => removed,
                        Err(error) => {
                            log::error!("Failed to process web accessibility actions: {error:#}");
                            return;
                        }
                    };
                    if removed {
...
impl Window {
    ...
    /// active pointer capture when this is the next frame of that element.
    pub(crate) fn register_pointer_capture_hitbox(
        &mut self,
    ...
    /// Produces a new frame and assigns it to `rendered_frame`. The internal
    ...
    pub(crate) fn handle_a11y_action(&mut self, request: accesskit::ActionRequest, cx: &mut App) {
        ...
        match request.action {
            ...
            accesskit::Action::Focus => {
                ...
                {
                    // A platform accessibility client expects SetFocus to move
                    // system keyboard focus, not only GPUI's internal dispatch
                    // focus. Activate the owning window before selecting its
